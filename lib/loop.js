...
function ensureFieldsAreSet(options) {
  if (!options.fields || _.isEmpty(options.fields)) {
    if (options.loop.subSteps) {
      options.fields = _.mapValues(
      _.keyBy(
          _.map(
            _.flatMap(options.loop.subSteps, step => step.fields),
            field => {
...
module.exports = superclass => class extends superclass {
  ...
  constructor(options) {
    ...
    this.options.loop.sectionKey = (this.options.route || '').replace(/\//g, '');
    this.options.loop.storeKey = this.options.loop.storeKey || `${this.options.loop.sectionKey}-items`;
  ...
  get(req, res, callback) {
    if (!req.params.action ||
      ...
      const step = this.hasItems(req) ?
        this.finalStep() :
        _.keys(this.options.loop.subSteps)[0];
  ...
  getBackLink(req, res) {
    const subSteps = _.intersection(
      Object.keys(this.options.loop.subSteps),
      [
  ...
  getNextStep(req, res) {
    ...
    const last = _.findKey(req.form.options.loop.subSteps, step => !step.next);
    ...
    const loopCondition = req.form.options.loop.loopCondition;
    if (stepName !== last) {
  ...
  saveValues(req, res, callback) {
    const steps = Object.keys(this.options.loop.subSteps);
    if (req.params.id) {
      ...
      req.sessionModel.set(this.options.loop.storeKey, items);
      return callback();
    }
  ...
  findSubStep(req) {
    return this.options.loop.subSteps[req.params.action];
  }
  ...
  finalStep() {
    return _.findKey(this.options.loop.subSteps, s => !s.next);
  }

  firstStep() {
    return _.keys(this.options.loop.subSteps)[0];
  }
  ...
  hasItems(req) {
    return _.size(req.sessionModel.get(this.options.loop.storeKey));
  }

  removeItem(req, res) {
    const items = req.sessionModel.get(this.options.loop.storeKey);
  ...
  getItems(req) {
    return req.sessionModel.get(req.form.options.loop.storeKey) || [];
  }
};
